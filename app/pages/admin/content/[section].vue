...
const makeForm=()=>({id:null as number|null,name:'',type:'casual',website:'',title:emptyText(),description:emptyText(),text:emptyText(),links:[] as any[],buttons:[] as any[],active:true,sort_order:0,recurring:false,valid_from:'',valid_to:'',imagePath:null as string|null})
...
async function save(){busy.value=true;error.value='';try{const body:any={active:form.value.active,sort_order:Number(form.value.sort_order)||0,recurring:form.value.recurring,valid_from:form.value.valid_from||null,valid_to:form.value.valid_to||null}; for(const f of cfg.value.localized) body[f]=(form.value as any)[f]; if(section.value==='restaurants') Object.assign(body,{name:form.value.name,type:form.value.type,website:form.value.website}); if(['faq','how-to'].includes(section.value)) body.links=form.value.links; if(section.value==='suggestions') body.buttons=form.value.buttons; await $fetch(form.value.id?`${api.value}/${form.value.id}`:api.value,{method:form.value.id?'PATCH':'POST',body}); editing.value=false;await refresh()}catch(e:any){error.value=e?.data?.statusMessage||e?.message||'Napaka pri shranjevanju'}finally{busy.value=false}}
async function remove(item:any){if(!confirm(`Res izbrisati ${cfg.value.single}?`))return;await $fetch(`${api.value}/${item.id}`,{method:'DELETE'});await refresh()}
...
<template>
  <div>
    ...
    <Teleport to="body"><div v-if="editing" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" @click.self="editing=false"><form class="max-h-[92vh] w-full max-w-2xl space-y-4 overflow-y-auto rounded-2xl bg-white p-6" @submit.prevent="save">
      ...
      <div v-if="['faq','how-to','suggestions'].includes(section)" class="space-y-3 rounded-xl border border-stone-200 p-4"><div class="flex items-center justify-between"><p class="text-sm font-medium text-stone-700">{{section==='suggestions'?'Gumbi':'Povezave'}}</p><button type="button" class="text-xs text-pine-700" @click="addAction">+ Dodaj</button></div><div v-for="(action,index) in (section==='suggestions'?form.buttons:form.links)" :key="index" class="space-y-2 rounded-lg bg-stone-50 p-3"><AdminLocalizedInput v-model="action.label" label="Besedilo gumba / povezave"/><div class="flex gap-2"><input v-model="action.href" type="url" placeholder="https://…" class="min-w-0 flex-1 rounded-lg border px-3 py-2 text-sm"/><button type="button" class="text-xs text-red-600" @click="removeAction(index)">Odstrani</button></div></div></div>
